#!/usr/bin/env python3
"""
Categorize 3497 image description texts into groups.
Each text can belong to multiple groups.
"""
...
# ─── Matching helper ──────────────────────────────────────────────────────────
def matches(text_lower, keywords):
    return any(kw in text_lower for kw in keywords)

# ─── Group definitions ────────────────────────────────────────────────────────
# Each group: list of lowercase substrings (any match → text belongs to group)
...
for text in texts:
    ...
    for group_name, keywords in GROUPS.items():
        if matches(tl, keywords):
            group_texts[group_name].append(text)
...
# Count how many groups each text belongs to
membership_counts = defaultdict(int)
for group_name, items in group_texts.items():
    for item in items:
        membership_counts[item] += 1

avg_membership = sum(membership_counts.values()) / len(texts) if texts else 0
print(f"Avg groups/text:  {avg_membership:.2f}")
print(f"\nGroup sizes (descending):")
for group, items in sorted(group_texts.items(), key=lambda x: len(x[1]), reverse=True):
    print(f"  {len(items):5d}  {group}")

if unassigned:
    print(f"\nUnassigned texts ({len(unassigned)}):")
    for t in unassigned[:80]:
        print(f"  - {t}")
...
output = {
    "metadata": {
        ...
        "average_groups_per_text": round(avg_membership, 2),
    },
    "groups": {name: sorted(items) for name, items in group_texts.items()},
...
print(f"\nSaved JSON to groups_output.json")
